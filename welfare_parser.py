...
class WelfareParser:
    ...
    # Field patterns for parsing - use [ \t]* to match only horizontal whitespace,
    # preventing blank fields from capturing content from the next line
    FIELD_PATTERNS = {
        'callsign': r'CALLSIGN:[ \t]*(.*)',
        'name': r'NAME:[ \t]*(.*)',
        'location': r'LOCATION:[ \t]*(.*)',
    ...
    # Hint text that should be treated as empty (template placeholders)
    HINT_TEXTS = {
        'callsign': ['or leave blank if not a licensed ham'],
        'status': ['safe / need assistance / traffic', '(safe / need assistance / traffic)'],
        'power': ['on / off / generator', '(on / off / generator)'],
        'contact': ['phone# for sms or email to notify family',
                     '(phone# for sms or email to notify family)'],
    }
        ...
        try:
            ...
            for field_name, pattern in self.FIELD_PATTERNS.items():
                ...
                if match:
                    ...
                    # Strip template hint/placeholder text
                    value = self._strip_hint_text(field_name, value)
    ...
    def _strip_hint_text(self, field_name, value):
        """
        Remove template placeholder/hint text from a parsed value.
        ...
        Returns:
            str: Cleaned value with hint text removed, or empty string
        """
        if not value:
            return value
        hints = self.HINT_TEXTS.get(field_name, [])
        value_lower = value.lower().strip()
        for hint in hints:
            if value_lower == hint.lower().strip():
                return ''
            # Also strip if the value starts with '(' and matches hint
            cleaned = value_lower.strip('() ')
            if cleaned == hint.lower().strip('() '):
                return ''
        return value
    ...
    def extract_callsign(self, text):
        ...
        match = re.search(r'CALLSIGN:[ \t]*([A-Z0-9]{3,7})', text, re.IGNORECASE)
        if match:
