#!/bin/bash
################################################################################
# Emcomm BBS - Unix/Linux/macOS Launcher
# Version 1.0.0
################################################################################
#
# Purpose: Launch the main Emcomm BBS GUI application
#
# Features:
...
if [ ! -f "emcomm_bbs.py" ]; then
    echo -e "${RED}  ERROR: emcomm_bbs.py not found!${NC}"
    MISSING_FILES=1
else
    echo -e "${GREEN}  ✓ emcomm_bbs.py found${NC}"
fi
