    ...
    def generate_text(self, window_info, checkins):
        ...
        try:
            ...
            for i, checkin in enumerate(sorted(checkins, key=lambda x: x.get('received_time', datetime.min)), 1):
                ...
                # Format identifier: use callsign(name) if ham, just name if non-ham
                if callsign:
                    id_str = f"{callsign}({name})"
                else:
                    id_str = f"[{name}]"
                ...
                lines.append(f"{i}. {id_str} {location}{power_str}{contact_str}{update_indicator}")
...
"""
            ...
            for i, checkin in enumerate(sorted(checkins, key=lambda x: x.get('received_time', datetime.min)), 1):
                ...
                # Display identifier: callsign for hams, [NAME] for non-hams
                callsign = checkin.get('callsign', '')
                name = checkin.get('name', 'Unknown')
                if callsign:
                    display_id = callsign
        ...
        try:
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                fieldnames = ['Date', 'Window', 'Callsign', 'Name', 'Location', 'Status',
                             'Power', 'Contact', 'Message', 'Received_Time', 'Update_Number', 'Previous_Status']
