...
class WelfareAggregator:
    ...
    def add_checkin(self, parsed_data, check_time=None):
        ...
        Supports both ham callsign and name-only (non-ham) check-ins.
        ...
        # Build identifier: callsign if available, else name-based
        identifier = self._get_identifier(parsed_data)
        
        # Check for existing check-ins from this identifier
        existing_index = None
        existing_checkin = None
        
        for i, existing in enumerate(self.checkins[window_key]):
            if self._get_identifier(existing) == identifier:
                existing_index = i
        ...
        if existing_checkin:
            ...
            if is_duplicate:
                ...
                display_id = parsed_data.get('callsign') or parsed_data.get('name', 'Unknown')
                return False, f"Duplicate: {display_id} - identical check-in already exists", window_info
            else:
                ...
                display_id = parsed_data.get('callsign') or parsed_data.get('name', 'Unknown')
                return True, f"Updated {display_id} in {window_info['name']} (update #{parsed_data['update_number']})", window_info
        ...
        display_id = parsed_data.get('callsign') or parsed_data.get('name', 'Unknown')
        return True, f"Added {display_id} to {window_info['name']}", window_info
    ...
    def _get_identifier(checkin_data):
        ...
        """
        callsign = (checkin_data.get('callsign') or '').strip().upper()
        if callsign:
            return callsign
        name = (checkin_data.get('name') or '').strip().upper()
    ...
    def get_summary(self):
        ...
        for window_key, checkins in self.checkins.items():
            ...
            summary.append({
                ...
                'callsigns': [c.get('callsign') or f"[{c.get('name', 'Unknown')}]" for c in checkins]
            })
