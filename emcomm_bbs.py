...
Generates ultra-compact plain-text situational-awareness reports for
transmission over low-bandwidth HF links (VarAC, Winlink, packet):
news headlines, NWS weather by FEMA region, NOAA space weather / HF band
conditions, emergency alerts, power outages, and a welfare check-in board.
"""
...
def _default_varac_dir(subfolder=""):
    """Best-effort default location for VarAC incoming files.

    Looks for a VarAC "Files in" folder in the usual places under the current
    user's profile. Falls back to the bundled data/ directories so the app is
    portable on any machine. The operator can always override with Browse.
    """
    from pathlib import Path as _P
    home = _P.home()
    candidates = [
        home / "Dropbox (Personal)" / "Ham Radio" / "Digital Modes" / "VarAC" / "Files in",
        home / "Dropbox" / "Ham Radio" / "Digital Modes" / "VarAC" / "Files in",
    ...
    for base in candidates:
        try:
            ...
            continue
    return _P("data") / {"": "input", "welfare_archive": "archive", "welfare_error": "error"}.get(subfolder, "input")
...
class NewsApp:
    ...
    def setup_welfare_tab(self, parent):
        ...
        default_welfare_dir = _default_varac_dir()
        ...
        default_archive_dir = _default_varac_dir("welfare_archive")
        ...
        default_error_dir = _default_varac_dir("welfare_error")
